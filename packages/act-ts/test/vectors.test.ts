/**
 * Test Vector Verification for ACT (TLS wire format)
 *
 * Reads test vectors from draft-schlesinger-02.json and verifies:
 * - Encoding roundtrips
 * - Full protocol flow with loaded values
 * - Parameter generation (H1-H4 determinism, distinctness)
 *
...
describe('ACT Vector Verification', () => {
  ...
  describe('parameter generation', () => {
    it('generates deterministic H1-H4 from domain separator', () => {
      const params1 = generateParameters(
        group,
        vectors.parameters.domain_separator,
        vectors.parameters.L
      );
      const params2 = generateParameters(
        group,
        vectors.parameters.domain_separator,
        vectors.parameters.L
      );
    ...
    it('generates non-identity H1-H4', () => {
      const params = generateParameters(
        group,
        vectors.parameters.domain_separator,
        vectors.parameters.L
      ...
      expect(params.H1.equals(group.identity())).toBe(false);
      expect(params.H2.equals(group.identity())).toBe(false);
      expect(params.H3.equals(group.identity())).toBe(false);
      expect(params.H4.equals(group.identity())).toBe(false);
    });

    it('generates distinct H1-H4', () => {
      const params = generateParameters(
        group,
        vectors.parameters.domain_separator,
        vectors.parameters.L
